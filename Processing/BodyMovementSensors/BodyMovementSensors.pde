...
void setup() {
  ...
  motion_data_a_x = new SensorDataList(previewResolution, 512);
  motion_data_a_y = new SensorDataList(previewResolution, 512);
  motion_data_a_z = new SensorDataList(previewResolution, 512);

  motion_data_b_x = new SensorDataList(previewResolution, 512);
  motion_data_b_y = new SensorDataList(previewResolution, 512);
  motion_data_b_z = new SensorDataList(previewResolution, 512);

  cardio_data = new SensorDataList(previewResolution, 0);
  pulse_data = new SensorDataList(previewResolution, 0);
  gsr_data = new SensorDataList(previewResolution, 0);
  stretch_data_a = new SensorDataList(previewResolution, 0);
  stretch_data_b = new SensorDataList(previewResolution, 0);
...
void serialEvent( Serial _port ) 
{
  ...
  if (myString != null) 
  {
    ...
    if ( state == READY && myString.equals("OK") ) 
    {
      ...
      {
        // save temp RAW data
        motion_data_a_x.add(acc[0]);
        motion_data_a_y.add(acc[1]);
        motion_data_a_z.add(acc[2]);
        motion_data_b_x.add(acc[3]);
        motion_data_b_y.add(acc[4]);
        motion_data_b_z.add(acc[5]);
        cardio_data.add(acc[6]);
        stretch_data_a.add(acc[7]);
        stretch_data_b.add(acc[8]);
        gsr_data.add(acc[9]);
        pulse_data.add(acc[10]);
